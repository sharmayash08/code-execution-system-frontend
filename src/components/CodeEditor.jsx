...
const CodeEditor = () => {
  ...
  const [code, setCode] = useState(
    localStorage.getItem("savedCode") || starterCode[language]
  );
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      setOutput(error , "Error executing code");
    }
    setLoading(false); 
  };
    ...
    <div className="min-h-screen bg-[#1E1E1E] text-white">
      ...
      <div className="grid grid-cols-2 gap-4 p-4">
        ...
        <div className="rounded-lg border border-gray-800 bg-[#1E1E1E]">
          ...
          {loading ? (
            <div className="flex h-[calc(100vh-180px)] items-center justify-center text-gray-400">
              <div className="text-center animate-pulse">
                <p>Executing Code...</p>
              </div>
            </div>
          ) : output ? (
            <pre className="p-4 text-white">{output}</pre>
          ) : (
            <div className="flex h-[calc(100vh-180px)] items-center justify-center text-gray-400">
